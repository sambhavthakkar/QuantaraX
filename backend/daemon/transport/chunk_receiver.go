...
func (r *ChunkReceiver) processChunkStream(stream *quic.Stream) {
	...
	if err != nil {
		...
		if r.logger != nil {
			r.logger.ChunkDecryptFailed(r.sessionID.String(), int(chunkIndex), "decrypt_failed", err.Error(), 0)
		}
	...
	if r.manifest != nil && chunkIndex >= 0 && int(chunkIndex) < len(r.manifest.Chunks) {
		...
		if computed != expected {
			...
			if r.metrics != nil {
				r.metrics.RecordChunkRetransmit("hash_mismatch")
			}
			if r.logger != nil {
			...
			case MessageTypeFECUpdate:
				...
				if json.Unmarshal(data, &u) == nil {
					...
					if r.fecDec != nil && (u.K > 0 && u.R > 0) {
						...
						if k > 0 && r.receivedCnt%int64(k) != 0 {
							apply = false
						}
						if apply {
				...
				if json.Unmarshal(data, &req) == nil {
					...
					if r.manifest != nil {
						for _, ch := range r.manifest.Chunks {
							if casHas(ch.Hash) {
								idxs = append(idxs, int64(ch.Index))
							}
						}
					}
					var comp ChunkRangeCompressor
