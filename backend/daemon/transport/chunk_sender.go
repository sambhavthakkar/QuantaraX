...
func (p *ChunkWorkerPool) SetChunkSize(bytes int64) { if bytes > 0 { p.chunkSize = bytes } }
	...
	for {
		...
		case chunkIndex, ok := <-p.chunkQueue:
			...
			if p.scheduler != nil {
				...
				p.scheduler.Enqueue(p.class, func(ctx context.Context) {
					if err := p.sendChunk(ci); err != nil {
						...
						if p.onChunkFailed != nil { p.onChunkFailed(ci, err) }
						return
					}
					if p.onChunkSent != nil { p.onChunkSent(ci) }
				})
				continue
			}
			...
			if err := p.sendChunk(chunkIndex); err != nil {
				...
				if p.onChunkFailed != nil { p.onChunkFailed(chunkIndex, err) }
				continue
			}
